...
def _extract_uuid_entries_from_tree(tree, dialect=None):
    """
    Collect uuid + marker candidates from a sqlglot tree.
    Marker candidates are used to match generated subexpressions/explanations.
    """
    entries = []
    seen = set()

    if not hasattr(tree, "walk"):
        return entries
    ...
    for node in tree.walk():
        ...
        # Main SQL text for this node.
        try:
            marker = node.sql(dialect=dialect) if dialect else node.sql()
            marker = marker.strip()
            if marker:
                markers.add(marker)
                # Also keep unquoted form for literal matching in NL.
        ...
        # Ensure Brace marker keeps `{...}` form.
        if node.__class__.__name__ == "Brace" and hasattr(node, "this") and node.this is not None:
            try:
                inner = node.this.sql(dialect=dialect) if dialect else node.this.sql()
                markers.add("{" + inner + "}")
            except Exception:
                pass
        ...
        key = (uid, tuple(sorted(markers)))
        if key in seen:
            continue
        seen.add(key)
        entries.append({"uuid": uid, "markers": list(markers)})

    return entries


def _try_extract_uuid_entries_from_sql(sql):
    """
    Best effort: parse brace SQL and collect uuid markers.
    If parse fails (or no braces), return [] without affecting main SQL2NL flow.
    """
    if "{" not in sql or "}" not in sql:
        return []

    try:
        from brace_toolkit import BraceSQLToolkit
        toolkit = BraceSQLToolkit()
        tree = toolkit.parse(sql)
        return _extract_uuid_entries_from_tree(tree, dialect=toolkit._BraceSQL)
    except Exception:
        return []


def _prepare_sql_and_uuid_entries(sql_input):
    """
    Accept either SQL string or a sqlglot tree (e.g. from brace_toolkit.parse).
    Return normalized SQL string and extracted uuid entries.
    """
    # Standard SQL string path.
    if isinstance(sql_input, str):
        return sql_input, _try_extract_uuid_entries_from_sql(sql_input)

    # Tree path (keeps upstream uuid if caller already parsed with brace_toolkit).
    sql_str = ""
    uuid_entries = []
    if hasattr(sql_input, "sql"):
        try:
            from brace_toolkit import CustomDialect
            sql_str = sql_input.sql(dialect=CustomDialect)
            uuid_entries = _extract_uuid_entries_from_tree(sql_input, dialect=CustomDialect)
        except Exception:
            try:
                sql_str = sql_input.sql()
            except Exception:
                sql_str = str(sql_input)
            uuid_entries = _extract_uuid_entries_from_tree(sql_input)
    else:
        sql_str = str(sql_input)

    return sql_str, uuid_entries


def _text_contains_uuid_marker(text, marker):
    if not marker or not text:
        return False

    # {name} in source SQL can become "{ name }" after SQL2NL preprocessing.
    if marker.startswith("{") and marker.endswith("}"):
        inner = marker[1:-1].strip()
        if inner:
            inner_parts = inner.split()
            inner_pattern = r"\s+".join(re.escape(p) for p in inner_parts) if inner_parts else re.escape(inner)
            brace_pattern = r"\{\s*" + inner_pattern + r"\s*\}"
            if re.search(brace_pattern, text, flags=re.IGNORECASE):
                return True

    # Literal quote style can change between single/double quote.
    if (marker.startswith("'") and marker.endswith("'")) or (marker.startswith('"') and marker.endswith('"')):
        raw = marker[1:-1].strip()
        if raw:
            quote_pattern = r"""['"]\s*""" + re.escape(raw) + r"""\s*['"]"""
            if re.search(quote_pattern, text):
                return True
            # Some NL outputs drop quote marks; fallback to token-boundary match.
            raw_tok_pattern = r"(?<![A-Za-z0-9_])" + re.escape(raw) + r"(?![A-Za-z0-9_])"
            if re.search(raw_tok_pattern, text):
                return True

    # Alnum markers use token boundary matching to reduce false positives.
    if re.fullmatch(r"[A-Za-z0-9_]+", marker):
        tok_pattern = r"(?<![A-Za-z0-9_])" + re.escape(marker) + r"(?![A-Za-z0-9_])"
        return bool(re.search(tok_pattern, text))
...
def _marker_regex_for_inline(marker):
    if not marker:
        return None
    ...
    if marker.startswith("{") and marker.endswith("}"):
        ...
        inner_parts = inner.split()
        inner_pattern = r"\s+".join(re.escape(p) for p in inner_parts) if inner_parts else re.escape(inner)
        return re.compile(r"\{\s*" + inner_pattern + r"\s*\}", flags=re.IGNORECASE)
    ...
    if (marker.startswith("'") and marker.endswith("'")) or (marker.startswith('"') and marker.endswith('"')):
        ...
        if not raw:
            return None
        return re.compile(
            r"""['"]\s*""" + re.escape(raw) + r"""\s*['"]|(?<![A-Za-z0-9_])""" + re.escape(raw) + r"""(?![A-Za-z0-9_])"""
        )

    if re.fullmatch(r"[A-Za-z0-9_]+", marker):
        return re.compile(r"(?<![A-Za-z0-9_])" + re.escape(marker) + r"(?![A-Za-z0-9_])")

    return re.compile(re.escape(marker))


def _normalize_markers_for_inline(markers):
    ...
    kept = []
    for m in cleaned:
        # Prefer quoted marker whenever both quoted/raw forms exist.
        if not ((m.startswith("'") and m.endswith("'")) or (m.startswith('"') and m.endswith('"'))):
            raw = m.strip()
            if raw in quoted_raw:
                continue
        if re.fullmatch(r"[A-Za-z0-9_]+", m) and m in quoted_raw:
            continue
        kept.append(m)

    return sorted(set(kept), key=lambda x: (-len(x), x))


def _annotate_text_inline_uuid(text, marker, uid):
    pattern = _marker_regex_for_inline(marker)
    if pattern is None:
        return text, False

    changed = False

    def _already_tagged(match_end: int) -> bool:
        i = match_end
        n = len(text)

        while i < n and text[i].isspace():
            i += 1
        if text.startswith("[uuid:", i):
            return True

        # Quoted spans are often rendered as: "value"[uuid:...]
    ...
    def _repl(match):
        nonlocal changed
        if _already_tagged(match.end()):
            return match.group(0)
        changed = True
        return match.group(0) + "[uuid:" + uid + "]"

    return pattern.sub(_repl, text), changed


def _inject_inline_uuid(subexpression, explanation, uuid_entries):
    if not explanation or not uuid_entries:
        return explanation
    ...
    for entry in uuid_entries:
        ...
        if not uid:
            continue
        uid_markers.setdefault(uid, [])
        uid_markers[uid].extend(entry.get("markers", []))

    for uid, raw_markers in uid_markers.items():
        markers = _normalize_markers_for_inline(raw_markers)
        if not markers:
            continue

        # Only annotate this uuid in current clause if marker appears in subexpression or explanation.
        if not any(_text_contains_uuid_marker(subexpression, m) or _text_contains_uuid_marker(text, m) for m in markers):
            continue

        inserted = False
        for marker in markers:
            if not (_text_contains_uuid_marker(subexpression, marker) or _text_contains_uuid_marker(text, marker)):
                continue
            text, changed = _annotate_text_inline_uuid(text, marker, uid)
            inserted = inserted or changed

        # If no concrete span was matched in explanation text, keep uuid visible as fallback.
        if not inserted and ("[uuid:" + uid + "]") not in text:
            text = text + " [uuid:" + uid + "]"
...
def sql2nl(sql):
    ...
    sql, uuid_entries = _prepare_sql_and_uuid_entries(sql)
    ...
    decompose(sql)
    reorganize_explanations()
    for unit in high_level_explanation:
        explanation = unit.get('explanation')
        if not isinstance(explanation, list) or len(explanation) != 2:
            continue
        sub_list, exp_list = explanation
        for idx in range(min(len(sub_list), len(exp_list))):
            exp_list[idx] = _inject_inline_uuid(sub_list[idx], exp_list[idx], uuid_entries)
    ...
    for i in range(len(result)):
        ...
        for j in range(len(result[i]['explanation'][0])):
            temp_sub = result[i]['explanation'][0][j]
            temp_exp = result[i]['explanation'][1][j]
            temp = {
                'subexpression': temp_sub,
