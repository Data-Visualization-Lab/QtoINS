...
from scipy.stats import (
    entropy, normaltest, mode, kurtosis, skew, pearsonr, moment
)
import pandas as pd
...
def aggregate_boolean_features(v, feature_name):
    r = OrderedDict([
        ('{}-agg-{}'.format(feature_name, c_aggregation_function), None)
    ...
    r['{}-agg-percentage'.format(feature_name)] = (
        sum(v) / len(v) if len(v) else None
    )
...
def aggregate_numeric_features(v, feature_name):
    r = OrderedDict([
        ('{}-agg-{}'.format(feature_name, q_aggregation_function), None)
    ...
    r['{}-agg-avg_abs_dev'.format(feature_name)] = np.mean(
        np.absolute(v - sample_mean)
    )
    r['{}-agg-med_abs_dev'.format(feature_name)] = np.median(
        np.absolute(v - sample_median)
    )
    r['{}-agg-coeff_var'.format(feature_name)] = (
    ...
    r['{}-agg-range'.format(feature_name)] = (
        sample_max - sample_min if (sample_max and sample_min) else None
    )
    r['{}-agg-normalized_range'.format(feature_name)] = (
        (sample_max - sample_min) / sample_mean
        if (sample_max and sample_min and sample_mean)
        else None
    )
